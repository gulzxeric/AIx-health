# RetinaEcho（数字记忆相框）系统设计文档

| 文档属性 | 详细信息 |
| :--- | :--- |
| **产品名称** | RetinaEcho（数字记忆相框） |
| **文档版本** | v1.0 |
| **更新日期** | 2026-09-05 |
...
RetinaEcho 是一款基于消费级硬件的无感多模态认知陪伴与神经生理筛查系统，面向轻中度阿尔茨海默病（AD）及轻度认知障碍（MCI）患者及其家庭照护者。
...
| **ASR** | 待定（FastAPI 旁路调用） | 普通话/英语，准确率 ≥ 90% |
| **TTS** | 待定（FastAPI 旁路调用） | 语速 0.85x，支持零样本克隆 |
| **LLM** | 待定（FastAPI 旁路调用） | 实体抽取 + 对话引擎 + 简报生成 |
| **人脸比对** | InsightFace / 云厂商 API | 闭集 5-10 人，准确率 ≥ 95% |
...
| **准确率要求** | ≥ 90% |
| **延迟要求** | ≤ 1.5s |
| **选型** | 待定（FastAPI 旁路调用，可替换） |

**集成方式**：FastAPI 异步调用 ASR 服务，语音文件先上传至 MinIO，ASR 读取后返回转写文本。
...
| 任务 | 周期 | 职责 |
| :--- | :--- | :--- |
| 每日简报生成 | 每日 02:00 | 计算活力指数 + 高共鸣话题 + LLM 建议 |
...
# LLM (待定)
LLM_ENDPOINT=http://localhost:8100
...
| **Phase 5** | `phase/5-daily-brief` | 埋点上报 API + 活力指数计算 + 简报生成批处理 + 简报查看 API | Phase 3 | 每日 02:00 生成简报 → 家属端可查看 |
